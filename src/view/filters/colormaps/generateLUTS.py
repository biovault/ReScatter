""" All of this code is a minor variation on https://github.com/derherrg/js-colormaps (MIT license)
    The difference is that for this project LUT.pngs are a more useful (i.e. network bandwidth efficient)
    way of delivering the color maps than a json array. We output pngs with the name of the color map.
    B. van Lew 25/Feb/2016
"""
...
def get_cm_values(colormap, n_shades, mapName):
    """
    ...
    """
    size = int(math.sqrt(n_shades) + 0.5)
    n_shades = size * size
    values = []
    ...
    for i in range(size):
        for j in range(size):
            color = values[i*size + j]
            r = int(math.floor(color[1][0] * 255 + 0.5))
            g = int(math.floor(color[1][1] * 255 + 0.5))
            b = int(math.floor(color[1][2] * 255 + 0.5))
            pixels[j,i] = (r, g, b, 255)
...
cmaps = {'PerceptuallyUniform': ['viridis', 'inferno', 'plasma', 'magma'],
...
plt.register_cmap(name='viridis', cmap=colmaps.viridis)
plt.register_cmap(name='inferno', cmap=colmaps.inferno)
plt.register_cmap(name='plasma', cmap=colmaps.plasma)
plt.register_cmap(name='magma', cmap=colmaps.magma)
...
for cm_type in cmaps.keys():
    ...
    for cmap in sorted(cmaps[cm_type]):
        values = get_cm_values(cm.get_cmap(cmap), 256, cmap) # must be a power of 2
