        ...
        dataMapper.mapData(context.sel.dataPoints, ReScatter.data.DataMapper.mapOp.PASSALLCOL)
            .then(values => {
                ...
                for (let i = 0, len = values.length; i < len; i++) {
                    ...
                    point['dataPointIndex'] = context.sel.dataPoints[i];
...
ReScatter.protocols.PlotProtocol.impl(RadvizPlotPlugin, [], {
     ...
     * @param points
     * @param props - the json props file in th eplot configuration
        ...
        this.radviz = radvizComponent()
            ...
            .on('dotEnter', function(d) {
                ...
                ReScatter.dynamicSelectionModel.putDynamicSelection(
                    'Radviz plot',
                    [d.dataPointIndex], //[pointIndex],
                    0x772222,
                    self.dynamicSelectionId,
                    {});
            })
            .on('dotLeave', function(/*d*/) {
                ReScatter.dynamicSelectionModel.removeDynamicSelection(self.dynamicSelectionId);
            });
        this.data = [];
        this.radviz.render(this.data);
        /**
...
ReScatter.protocols.SnapshotProtocolForSVG.impl(RadvizPlotPlugin, [], {
    ...
    getSVGTextAtIndex(index) {
        if (index === 0) {
            // just want the SVG not the popup div
            return document.querySelector('#' + this.elementId).children[0].outerHTML;
        }
        return ''; // No svgs
    },
    ...
    getCSSTextAtIndex(index) {
        if (index === 0) {
            for (let sheet of document.styleSheets) {
                // Find the radviz style sheet used to style this control
                if (sheet.href.includes('RadvizPlotPlugin.css')) {
                    // extract and concatenate the css text for all rules
                    return Array.from(sheet.cssRules).map(x => x.cssText).join(' ');
                }
            }
            return '';
        }
